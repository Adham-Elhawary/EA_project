...
# variables
############
video_path = r"D:\W_W - OIIA OIIA (Spinning Cat)(720P_HD).mp4"
y = 1
NUM_DAYS = 5
NUM_HOURS = 10
...
while y == 1 :
                ...
                if NUM_COURSES < 0 or NUM_COURSES is None :
                    ...
                    if NUM_LECTURERS < 0 or NUM_LECTURERS is None :
                        ...
                        if NUM_STUDENTS  < 0 or NUM_STUDENTS  is None:
                            print("Invalid Input")
                            continue
                        else:
...
class Student:
    def __init__(self, id, name=None):
        self.id = id
        self.name = name if name else f"Student {self.id}"
...
class UniversityTimetablingProblem:
    ...
    def generate_students(self):
        ...
        try:
            with open('students_list.csv', mode='r') as file:
                # Try different possible column names for ID
                possible_id_columns = ['id', 'ID', 'student_id', 'Student ID', 'Id']
                possible_name_columns = ['name', 'Name', 'student_name', 'Student Name']
                ...
                if not id_column:
                    raise ValueError("No valid ID column found in CSV file")

                for row in reader:
                    try:
                        # Get ID (required)
                        student_id = int(row[id_column])

                        # Get name (optional)
                        student_name = row[name_column] if name_column else f"Student {student_id}"
                        ...
                        print(f"Error processing student data (ID: {row.get(id_column, 'N/A')}): {e}")
                        continue

        except FileNotFoundError:
            print("students_list.csv not found. Creating sample students instead.")
            # Fallback to creating sample students if file not found
            start_id = 2022000
        ...
        # Verify we have enough students
        if len(students) < NUM_STUDENTS:
            print(
                f"Warning: Only found {len(students)} students in CSV, but need {NUM_STUDENTS}. Adding dummy students.")
            start_id = max([s.id for s in students], default=2022000) + 1
            for i in range(NUM_STUDENTS - len(students)):
                student_id = start_id + i
                student = Student(student_id)
                students.append(student)
        ...
        for student in students[:5]:
            ...
            print(f"Student {student.id}: {len(student.courses)} courses")
    ...
    def evaluate_timetable(self, timetable):
        ...
        if not timetable.assignments:
            ...
            penalties = {
                ...
                'student_conflict': 1,  # Reduced from 100 to 1 per student overlap
                'room_capacity': 50,
...
# Gui \ loading screen
############
# [Previous imports and class definitions remain the same until the GUI section]

class VideoLoadingScreen:
    def __init__(self, root, video_path= video_path):
        self.root = root
...
class TimetablingGUI:
    ...
    def show_loading_screen(self, message):
        self.loading_screen = VideoLoadingScreen(self.root)
